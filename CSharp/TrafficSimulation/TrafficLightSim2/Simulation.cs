        ...
        public void run()
        {
            ...
            if (frameCount % Settings.CAR_FREQUENCY == 0 || frameCount == 1)
            {
                ...
                if (randGen.Next(100) < (Settings.getSimSettings().getHCarProbability() * 100))
                {
                    ...
                    int randNum = randGen.Next(100);
                    if (randNum < (Settings.getSimSettings().getTurnLeftProbability() * 100))
                    {
                        workingCar.setTurningLeft(true);
                    }

                    randNum = randGen.Next(100);
                    if (randNum < (Settings.getSimSettings().getTurnRightProbability() * 100))
                    {
